...
mvector Increment_find_motion(my_image_comp* ref, my_image_comp* tgt, int start_row, int start_col,int start_origion_row,
int star_origion_col,int block_width, int block_height, int S) {
    float N = (float)block_height * (float)block_width;
    mvector vec, best_vec;//vec就是运动矢量
    int mse, best_mse = 256 * block_width * block_height;
    for (vec.y_ =  -3; vec.y_ <=  3; vec.y_ +=1)
        for (vec.x_ = -3; vec.x_ <= 3; vec.x_ +=1)
        {
            int ref_row = start_row - vec.y_;
            int ref_col = start_col - vec.x_;
            if ((ref_row < 0) || (ref_col < 0) ||
...
mvector Half_pixel_find_motion(my_image_comp* ref, my_image_comp* tgt, 
    int start_row, int start_col, int start_origion_row, int star_origion_col, int block_width, int block_height, int S)

{
    ...
    int mse, best_mse = 256 * block_width * block_height;
    //Filter sinc(7,7);
    for (vec.y = - 0.5; vec.y <=  0.5; vec.y += 0.5)
        for (vec.x =  - 0.5; vec.x <=  0.5; vec.x += 0.5)
        {
